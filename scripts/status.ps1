...
$node = Get-NetTCPConnection -LocalPort 9528 -State Listen -ErrorAction SilentlyContinue
...
Write-Host ("Node 9528 : " + ($(if ($node) { "running" } else { "stopped" })))
Write-Host ("Bridge    : " + ($(if ($bridge) { "running pid=$($bridge.ProcessId)" } else { "stopped" })))
Write-Host ("MT5       : " + ($(if ($mt5) { "running" } else { "stopped" })))

if ($node) {
  try {
    $health = Invoke-RestMethod -Uri "http://127.0.0.1:9528/health" -TimeoutSec 3
    Write-Host "Quotes    : $($health.quotes)"
    if ($health.quotes -gt 0) {
      $list = Invoke-RestMethod -Uri "http://127.0.0.1:9528/quotes" -TimeoutSec 3
      foreach ($q in @($list)) {
        Write-Host ("  {0} bid={1} ask={2}" -f $q.symbol, $q.bid, $q.ask)
      }
      Write-Host "Browser   : http://127.0.0.1:9528/"
    } else {
